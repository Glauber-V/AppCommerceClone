...
class ProductFullDetailFragmentLocalTest {

    @get:Rule(order = 0)
    var hiltAndroidRule = HiltAndroidRule(this)

    @get:Rule(order = 1)
    var testNavHostControllerRule = TestNavHostControllerRule(R.id.product_full_detail_fragment)
    ...
    fun verifyProductWasLoaded_ShouldPass() {
        launchFragmentInHiltContainer<ProductFullDetailFragment>(navHostController = navHostController) {
            productViewModel.selectProduct(product)
        }

        onView(withId(R.id.product_full_detail_name))
            .check(matches(withText(product.name)))

        val formattedPrice = NumberFormat.getCurrencyInstance().format(product.price)
        onView(withId(R.id.product_full_detail_price))
            .check(matches(withText(formattedPrice)))

        onView(withId(R.id.product_full_detail_description))
    ...
    fun clickAddToFavorites_shouldNavigateToFavoritesFragment() {
        ...
        onView(withId(R.id.product_full_detail_add_to_favorites))
            .perform(scrollTo())
            .perform(click())
    ...
    fun clickAddToCart_shouldNavigateToCartFragment() {
        ...
        onView(withId(R.id.product_full_detail_add_to_cart))
            .perform(scrollTo())
            .perform(click())
    ...
    fun clickBuyNow_snackbarShouldBeVisible() {
        ...
        onView(withId(R.id.product_full_detail_buy_now))
            .perform(scrollTo())
            .perform(click())
